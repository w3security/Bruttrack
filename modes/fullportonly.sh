...
if [[ "$MODE" = "fullportonly" ]]; then
  if [[ "$REPORT" = "1" ]]; then
    ...
    echo "bruttrack -t $TARGET -m $MODE --noreport " >> $LOOT_DIR/scans/running_${TARGET}_${MODE}.txt
  ...
  if [[ -f "/usr/share/bruttrack/pro/.portscanner.conf" ]]; then
    source /usr/share/bruttrack/pro/.portscanner.conf
  fi
  if [[ -z "$PORT" ]]; then
