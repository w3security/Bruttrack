...
if [[ "$MODE" = "webporthttp" ]]; then
  if [[ "$REPORT" = "1" ]]; then
    ...
    echo "[log4j.codes] •?((¯°·._.• Started Bruttrack scan: http://$TARGET:$PORT [$MODE] (`date +"%Y-%m-%d %H:%M"`) •._.·°¯))؟•" >> $LOOT_DIR/scans/notifications_new.txt
    if [[ "$SLACK_NOTIFICATIONS" == "1" ]]; then
      /bin/bash "$INSTALL_DIR/bin/slack.sh" "[log4j.codes] •?((¯°·._.• Started Bruttrack scan: http://$TARGET:$PORT [$MODE] (`date +"%Y-%m-%d %H:%M"`) •._.·°¯))؟•"
    fi
  ...
  if [[ -z "$port_http" ]]; then
    ...
    if [[ "$JEXBOSS" == "1" ]]; then
      ...
      echo -e "${OKGREEN}====================================================================================${RESET}•x${OKGREEN}[`date +"%Y-%m-%d](%H:%M)"`${RESET}x•"
      cd /tmp/
      python /usr/share/bruttrack/plugins/jexboss/jexboss.py -u http://$TARGET:${PORT} | tee $LOOT_DIR/web/jexboss-$TARGET-port${PORT}.raw
    ...
    if [[ "$SMUGGLER" = "1" ]]; then
      ...
      echo -e "${OKGREEN}====================================================================================${RESET}•x${OKGREEN}[`date +"%Y-%m-%d](%H:%M)"`${RESET}x•"
      python3 /usr/share/bruttrack/plugins/smuggler/smuggler.py --no-color -u http://$TARGET:${PORT} | tee $LOOT_DIR/web/smuggler-$TARGET-port${PORT}.txt
    fi
    if [[ "$NUCLEI" = "1" ]]; then
      echo -e "${OKGREEN}====================================================================================${RESET}•x${OKGREEN}[`date +"%Y-%m-%d](%H:%M)"`${RESET}x•"
  ...
  echo "[i] Scanning: http://$TARGET:$PORT/"
    sudo python3 /usr/share/bruttrack/bin/zap-scan.py "http://$TARGET:$PORT/" 
    DATE=$(date +"%Y%m%d%H%M")
    sudo grep "'" /usr/share/bruttrack/bin/zap-report.txt | cut -d\' -f2 | cut -d\\ -f1 > $LOOT_DIR/web/zap-report-$TARGET-http-$DATE.html
  ...
  echo "[log4j.codes] •?((¯°·._.• Finished Bruttrack scan: http://$TARGET:$PORT [$MODE] (`date +"%Y-%m-%d %H:%M"`) •._.·°¯))؟•" >> $LOOT_DIR/scans/notifications_new.txt
  if [[ "$SLACK_NOTIFICATIONS" == "1" ]]; then
    /bin/bash "$INSTALL_DIR/bin/slack.sh" "[log4j.codes] •?((¯°·._.• Finished Bruttrack scan: http://$TARGET:$PORT [$MODE] (`date +"%Y-%m-%d %H:%M"`) •._.·°¯))؟•"
  fi
  if [[ "$LOOT" = "1" ]]; then
    loot
  fi  
