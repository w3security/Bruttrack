if [[ "$MODE" = "webscan" ]]; then
    ...
    if [[ "$SLACK_NOTIFICATIONS" == "1" ]]; then
      ...
      echo "[i] Scanning: http://$TARGET/"
        sudo python3 /usr/share/bruttrack/bin/zap-scan.py "http://$TARGET/" 
        DATE=$(date +"%Y%m%d%H%M")
        ...
        sudo python3 /usr/share/bruttrack/bin/zap-scan.py "https://$TARGET/"
    ...
    if [[ "$NUCLEI" = "1" ]]; then
      ...
      echo -e "${OKGREEN}====================================================================================${RESET}•x${OKGREEN}[`date +"%Y-%m-%d](%H:%M)"`${RESET}x•"
      nuclei -silent -t /usr/share/bruttrack/plugins/nuclei-templates/ -c $THREADS -target http://$TARGET -o $LOOT_DIR/web/nuclei-http-${TARGET}-port80.txt
      nuclei -silent -t /usr/share/bruttrack/plugins/nuclei-templates/ -c $THREADS -target https://$TARGET -o $LOOT_DIR/web/nuclei-https-${TARGET}-port443.txt 
    fi
